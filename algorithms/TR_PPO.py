...
def algorithm(args, memory, network):
    ...
    for i_epoch in range(int(args.num_epoch * batch_size / args.minibatch_size)):
        ...
        kl = F.kl_div(minibatch_newlogproba.softmax(dim=-1).log(), minibatch_oldlogproba.softmax(dim=-1),
                      reduction='sum')
        surr2 = torch.where(kl <= 0.03, ratio,
                            torch.where(torch.logical_and(ratio > 1, minibatch_advantages > 0), ratio.clamp(1, 1),
                                        torch.where(torch.logical_and(ratio < 1, minibatch_advantages < 0),
                                                    ratio.clamp(1, 1),
